...
def basic_conv(in_tensor, value_dict, layer_num, repeat_num, conv_num,
             flag_shortcut=False, down_sample=False, weight_shorcut = None, name_bn_shorcut =None, is_training=False, res_in_tensor=None):
...
def basic_block(in_tensor, value_dict, layer_num, repeat_num, down_sample=False, is_training=False):
    ...
    tensor_go_next =  basic_conv(in_tensor, value_dict, layer_num, repeat_num, conv_num=1,
                                 flag_shortcut=False, down_sample=False, weight_shorcut = None, name_bn_shorcut =None, res_in_tensor= None)
    tensor_go_next =  basic_conv(tensor_go_next, value_dict, layer_num, repeat_num, conv_num=2,
                                 flag_shortcut=True, down_sample=down_sample, weight_shorcut = None, name_bn_shorcut =None, res_in_tensor = in_tensor)
    return tensor_go_next
...
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=tensor_go_next_fc, labels=y))
...
with tf.Session() as sess:
    ...
    saver.save(sess, 'resnet34_pytorch', write_meta_graph = False)
